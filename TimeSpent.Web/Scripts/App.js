...
.constant('BaseApi', TimeSpent.rootPath);      // not used in the code yet
...
commonModule.factory('viewModelHelper', function ($http, $q , BaseApi) {
    return TimeSpent.viewModelHelper($http, $q, BaseApi);
});
...
(function (cr) {
    var viewModelHelper = function ($http, $q , BaseApi) {
        ...
        self.apiGet = function (uri, data, success, failure, always) {
            self.isLoading = true;
            self.modelIsValid = true;
            $http.get(TimeSpent.rootPath + uri, data)
        ...
        self.apiPost = function (uri, data, success, failure, always) {
            self.isLoading = true;
            self.modelIsValid = true;
            $http.post(TimeSpent.rootPath + uri, data)
                .then(function (result) {
                    success(result);
                    if (always != null)
                        always();
...
(function (val) {
    var validator = function () {
                    ...
                    if (prop == propertyRule.PropertyName) {
                        ...
                        for (var k = 0; k < propertyRuleProps.length; k++)
                        {
                            ...
                            if (propertyRuleProp != 'custom') {
                                ...
                                if (!validationResult) {
                                   // errors.push(getMessage(prop, propertyRuleProp, rule.message));   ( fixing the custom message bug)
        ...
        var setupRules = function () {
            ...
            rules['range'] = {
                validator: function (value, params) {
                    var number = Number.parseInt(value.trim() || '');
                    if (!number) return false;
                    var regExp = new RegExp(/[0-9]/);
                    return !(regExp.exec(value.trim()) == null);
                    var min = Number.parseInt(params.min);
                    var max = Number.parseInt(params.max);
                    return !(number < min || number > max);
                },
                message: 'Value must be between x and y.'
            };
        }
